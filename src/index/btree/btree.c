...
{
    if (!s.len || !s.data) return FASTKV_SLICE_NULL;
    void *p = fkv_malloc(s.len);
    if (!p) return FASTKV_SLICE_NULL;
...
static split_t leaf_insert(lnode_t *l, fastkv_slice_t key, fastkv_slice_t val)
{
    ...
    if (l->n < LEAF_MAX) {
        memmove(&l->k[i + 1], &l->k[i], (l->n - i) * sizeof(fastkv_slice_t));
        memmove(&l->v[i + 1], &l->v[i], (l->n - i) * sizeof(fastkv_slice_t));
        l->k[i] = sdup(key);
        l->v[i] = sdup(val);
        l->n++;
        return NO_SPLIT;
...
static split_t intr_insert(inode_t *p, fastkv_slice_t key, fastkv_slice_t val)
{
    ...
    if (!s.right) return NO_SPLIT;

    /* anak ci baru saja split — sisipkan pemisah baru */
    if (p->n < INTR_MAX) {
        memmove(&p->k[ci + 1], &p->k[ci], (p->n - ci) * sizeof(fastkv_slice_t));
        memmove(&p->c[ci + 2], &p->c[ci + 1], (p->n - ci) * sizeof(void *));
        p->k[ci] = s.sep;
        p->c[ci + 1] = s.right;
        p->n++;
        return NO_SPLIT;
...
static split_t node_insert(void *node, fastkv_slice_t key, fastkv_slice_t val)
{
    if (*(nkind_t *)node == NLEAF) return leaf_insert(node, key, val);
...
fastkv_err_t fastkv_btree_delete(fastkv_btree_t *t, fastkv_slice_t key)
{
    ...
    sfree(l->k[i]);
    sfree(l->v[i]);
    memmove(&l->k[i], &l->k[i + 1], (l->n - i - 1) * sizeof(fastkv_slice_t));
    memmove(&l->v[i], &l->v[i + 1], (l->n - i - 1) * sizeof(fastkv_slice_t));
    l->n--;
    ...
    if (dir == FASTKV_CURSOR_FORWARD) {
        ...
        while (l) {
            for (int i = start; i < l->n; i++) {
                if (max.data && cmp(l->k[i], max) > 0) goto done;
        ...
        while (l) {
            for (int i = start; i >= 0; i--) {
                if (min.data && cmp(l->k[i], min) < 0) goto done;
                rc = cb(l->k[i], l->v[i], udata);
                if (rc != FASTKV_OK) goto done;
            }
            l = l->prev;
