...
fastkv_err_t fastkv_open(fastkv_db_t **db_out, const fastkv_opts_t *opts)
{
    ...
    if (!opts->read_only) fastkv_compaction_start(db);
...
fail:
    if (db->btree) fastkv_btree_destroy(db->btree);
...
fastkv_err_t fastkv_close(fastkv_db_t *db)
{
    ...
    if (db->wal) fastkv_checkpoint(db);
    ...
    fastkv_db_t *db, fastkv_write_entry_t *head, fastkv_ts_t begin_ts)
{
    ...
    for (fastkv_write_entry_t *e = head; e; e = e->next) {
        ...
        while (v) {
            if (v->key.len == e->key.len &&
                memcmp(v->key.data, e->key.data, e->key.len) == 0)
            {
...
fastkv_err_t fastkv_cursor_open(
    fastkv_txn_t *txn, fastkv_cursor_dir_t dir, fastkv_cursor_t **out)
{
    if (!txn || !out) return FASTKV_ERR_INVAL;

    fastkv_cursor_t *c = fkv_malloc(sizeof(*c));
    if (!c) return FASTKV_ERR_NOMEM;
    memset(c, 0, sizeof(*c));
    c->db = txn->db;
    c->dir = dir;

    snap_t s = {NULL, NULL, 0, 0};
    fastkv_err_t rc = fastkv_btree_scan(txn->db->btree,
        FASTKV_SLICE_NULL, FASTKV_SLICE_NULL,
...
fastkv_err_t fastkv_cursor_seek(fastkv_cursor_t *c, fastkv_slice_t key)
{
    ...
    while (lo < hi) {
        ...
        if (!cmp_res) cmp_res = (c->keys[mid].len > key.len) - (c->keys[mid].len < key.len);
        if (cmp_res < 0) lo = mid + 1;
        else hi = mid;
    }

    if (lo >= c->n) return FASTKV_ERR_CURSOR_EOF;
    c->pos = lo;
    return FASTKV_OK;
}
...
fastkv_err_t fastkv_cursor_next(fastkv_cursor_t *c)
{
    ...
    if (c->dir == FASTKV_CURSOR_FORWARD) {
        c->pos++;
        if (c->pos >= c->n) return FASTKV_ERR_CURSOR_EOF;
    } else {
        c->pos--;
        if (c->pos < 0) return FASTKV_ERR_CURSOR_EOF;
    }
    return FASTKV_OK;
}
...
{
    if (!c || !out) return FASTKV_ERR_INVAL;
    if (c->pos < 0 || c->pos >= c->n) return FASTKV_ERR_CURSOR_EOF;
    *out = c->keys[c->pos];
    return FASTKV_OK;
}
...
/* index sekunder — stub Phase 3+ */

fastkv_err_t fastkv_index_create(fastkv_db_t *db, const char *name,
    fastkv_index_fn fn, void *udata, fastkv_index_t **index)
{
