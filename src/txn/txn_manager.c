...
/* Forward declarations (implemented in kv_api.c) */
struct fastkv_db;
...
fastkv_err_t fastkv_txn_mgr_begin(
    fastkv_txn_mgr_t *mgr, struct fastkv_db *db, bool read_only, fastkv_txn_t **txn_out) {
    ...
    int slot;
    txn->begin_ts    = fastkv_oracle_begin(&mgr->oracle, &slot);
    txn->oracle_slot = slot;
    ...
    LOG_TRACE("TXN begin ts=%" PRIu64 " slot=%d ro=%d", txn->begin_ts, slot, read_only);
    *txn_out = txn;
    return FASTKV_OK;
}
...
fastkv_err_t fastkv_txn_mgr_commit(fastkv_txn_mgr_t *mgr, fastkv_txn_t *txn) {
    ...
    atomic_fetch_add_explicit(&mgr->num_committed, 1, memory_order_relaxed);
    fastkv_oracle_release(&mgr->oracle, txn->oracle_slot);
...
fastkv_err_t fastkv_txn_mgr_abort(fastkv_txn_mgr_t *mgr, fastkv_txn_t *txn) {
    if (txn->state == TXN_ABORTED)
        return FASTKV_OK;

    txn->state = TXN_ABORTED;
    atomic_fetch_add_explicit(&mgr->num_aborted, 1, memory_order_relaxed);
    fastkv_oracle_release(&mgr->oracle, txn->oracle_slot);
    fastkv_arena_destroy(txn->arena);
    fkv_free(txn);
    return FASTKV_OK;
