...
fastkv_err_t fastkv_wal_open(fastkv_wal_t **wal, const char *dir, bool sync_writes) {
    ...
    if (d) {
        ...
        while ((ent = readdir(d)) != NULL) {
            if (strncmp(ent->d_name, WAL_FILENAME_PFX, strlen(WAL_FILENAME_PFX)) != 0) continue;
            if (!strstr(ent->d_name, WAL_FILENAME_SFX)) continue;
            uint64_t id = (uint64_t)strtoull(ent->d_name + strlen(WAL_FILENAME_PFX), NULL, 10);
            if (id >= w->segment_id) w->segment_id = id;
        }
        closedir(d);
    }
    ...
    LOG_DEBUG("WAL opened: segment %" PRIu64 " (%s) offset=%" PRIu64, w->segment_id, path, w->file_offset);
    *wal = w;
    return FASTKV_OK;
}
...
fastkv_err_t fastkv_wal_append(fastkv_wal_t *wal, fastkv_wal_rec_type_t type, fastkv_ts_t ts,
    ...
    fastkv_err_t rc;
    if ((rc = wal_write_buf(wal, &crc, 4)) != FASTKV_OK) return rc;
    if ((rc = wal_write_buf(wal, &t, 1)) != FASTKV_OK) return rc;
    if ((rc = wal_write_buf(wal, &ts, 8)) != FASTKV_OK) return rc;
    if ((rc = wal_write_buf(wal, &klen, 4)) != FASTKV_OK) return rc;
    if ((rc = wal_write_buf(wal, &vlen, 4)) != FASTKV_OK) return rc;
    if (key.len && (rc = wal_write_buf(wal, key.data, key.len)) != FASTKV_OK) return rc;
    if (value.len && (rc = wal_write_buf(wal, value.data, value.len)) != FASTKV_OK) return rc;
...
static fastkv_err_t replay_segment(const char *path, fastkv_ts_t since_ts, fastkv_wal_replay_fn fn,
    ...
    for (;;) {
        ...
        if (!read_u32(f, &stored_crc)) break;
        if (!read_u8(f, &type_byte)) break;
        if (!read_u64(f, &ts)) break;
        if (!read_u32(f, &klen)) break;
        ...
        if (klen && fread(key_buf, 1, klen, f) != klen) break;
        if (real_vlen && fread(val_buf, 1, real_vlen, f) != real_vlen) break;
        ...
        if (klen) computed = fastkv_crc32c(computed, key_buf, klen);
        if (real_vlen) computed = fastkv_crc32c(computed, val_buf, real_vlen);
        ...
        if (type_byte != WAL_REC_PUT && type_byte != WAL_REC_DELETE) continue;
        ...
        fastkv_wal_record_t rec = {
            ...
            .value = (vlen == WAL_VLEN_TOMBSTONE) ? FASTKV_SLICE_NULL : FASTKV_SLICE(val_buf, real_vlen),
        };
...
fastkv_err_t fastkv_wal_replay(const char *dir, fastkv_ts_t since_ts, fastkv_wal_replay_fn fn,
    ...
    for (size_t i = 0; i < count; i++) {
        ...
        if (rc != FASTKV_OK) break;
    }

    LOG_INFO("WAL replay: %zu segments, %" PRIu64 " records replayed, max_ts=%" PRIu64, count, total, max_ts);

    if (max_ts_out) *max_ts_out = max_ts;
    fkv_free(ids);
    return rc;
}
...
fastkv_err_t fastkv_wal_rotate(fastkv_wal_t *wal) {
    flush_buf(wal, true);
    if (wal->io) fastkv_io_close(wal->io);
...
fastkv_err_t fastkv_wal_trim(const char *dir, uint64_t keep_from_id) {
    ...
    uint64_t removed = 0;
    for (size_t i = 0; i < count; i++) {
        if (ids[i] >= keep_from_id) continue;
        snprintf(path, sizeof path, WAL_FILENAME_FMT, dir, ids[i]);
...
uint64_t fastkv_wal_current_segment(fastkv_wal_t *wal) { return wal ? wal->segment_id : 0; }
uint64_t fastkv_wal_bytes_written(fastkv_wal_t *wal) { return wal ? wal->bytes_written : 0; }
