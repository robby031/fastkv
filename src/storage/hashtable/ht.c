...
{
 if (s.len == 0) return FASTKV_SLICE_NULL;
 uint8_t *buf = fkv_malloc(s.len);
 if (!buf) return FASTKV_SLICE_NULL;
...
fastkv_err_t fastkv_ht_create(fastkv_ht_t **ht, size_t initial_capacity)
{
 ...
 if (!h) return FASTKV_ERR_NOMEM;

 h->buckets = fkv_malloc(initial_capacity * sizeof(*h->buckets));
 if (!h->buckets) { fkv_free(h); return FASTKV_ERR_NOMEM; }
...
void fastkv_ht_destroy(fastkv_ht_t *ht)
{
 ...
 if (v->value.data) fkv_free((void *)v->value.data);
 fkv_free(v);
 v = next;
 }
 }
...
fastkv_err_t fastkv_ht_get(fastkv_ht_t *ht, fastkv_ts_t snapshot_ts, fastkv_slice_t  key, fastkv_slice_t *value_out)
{
 ...
 while (v) {
 if (slice_eq(v->key, key) &&
 v->begin_ts <= snapshot_ts &&
 snapshot_ts <  v->end_ts)
 {
 if (!v->value.data) return FASTKV_ERR_NOTFOUND;  /* tombstone */
 *value_out = v->value;
 return FASTKV_OK;
 }
...
static fastkv_err_t ht_insert_version(fastkv_ht_t *ht, fastkv_ts_t commit_ts, fastkv_slice_t key, fastkv_slice_t value /* NULL = tombstone */)
{
 fastkv_version_t *nv = fkv_malloc(sizeof(*nv));
 if (!nv) return FASTKV_ERR_NOMEM;
