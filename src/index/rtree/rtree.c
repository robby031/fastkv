...
static void rect_expand(
    fastkv_coord_t *dst_mn, fastkv_coord_t *dst_mx,
    const fastkv_coord_t *src_mn, const fastkv_coord_t *src_mx, int ndims)
{
    for (int i = 0; i < ndims; i++) {
        if (src_mn[i] < dst_mn[i]) dst_mn[i] = src_mn[i];
        if (src_mx[i] > dst_mx[i]) dst_mx[i] = src_mx[i];
    }
}
...
{
    for (int i = 0; i < ndims; i++) {
        if (r->min[i] < b->min[i] || r->max[i] > b->max[i]) return false;
    }
    return true;
}
...
{
    for (int i = 0; i < ndims; i++) {
        if (r->max[i] < b->min[i] || r->min[i] > b->max[i]) return false;
    }
    return true;
}
...
static double mindist_sq(const fastkv_coord_t *pt, const rentry_t *e, int ndims)
{
    ...
    for (int i = 0; i < ndims; i++) {
        double v = pt[i];
        if (v < e->min[i]) v = e->min[i];
        else if (v > e->max[i]) v = e->max[i];
        double diff = pt[i] - v;
        d += diff * diff;
    }
    return d;
...
static void linear_split(rnode_t *full, rentry_t *new_e, rnode_t *left, rnode_t *right, int ndims)
{
    ...
    rentry_t all[RT_M + 1];
    for (int i = 0; i < full->n; i++) all[i] = full->e[i];
    all[full->n] = *new_e;
    int total = full->n + 1;
    ...
    for (int i = 0; i < total - 1; i++) {
        for (int j = i + 1; j < total; j++) {
            ...
            double waste = combined
                - rect_area(all[i].min, all[i].max, ndims)
    ...
    for (int d = 0; d < ndims; d++) {
        lmn[d] = all[seed1].min[d]; lmx[d] = all[seed1].max[d];
        rmn[d] = all[seed2].min[d]; rmx[d] = all[seed2].max[d];
    }
...
static rsplit_t rtree_insert_leaf(rnode_t *leaf, rentry_t *e, int ndims)
{
    ...
    leaf->n = left_tmp->n;
    for (int i = 0; i < left_tmp->n; i++) leaf->e[i] = left_tmp->e[i];
    fkv_free(left_tmp);
...
static rsplit_t rtree_insert_internal(rnode_t *node, rentry_t *e, int ndims)
{
    ...
    node->n = left_tmp->n;
    for (int i = 0; i < left_tmp->n; i++) node->e[i] = left_tmp->e[i];
    fkv_free(left_tmp);
...
static rsplit_t rtree_insert_rec(rnode_t *node, rentry_t *e, int ndims)
{
    if (node->is_leaf) return rtree_insert_leaf(node, e, ndims);
    return rtree_insert_internal(node, e, ndims);
}

/* hapus entry dari subtree — kembalikan true jika berhasil ditemukan */
static bool rtree_delete_rec(rnode_t *node, const fastkv_rect_t *rect,
    const uint8_t *key_data, size_t key_len, int ndims)
{
    if (node->is_leaf) {
        for (int i = 0; i < node->n; i++) {
            if (node->e[i].key.len != key_len) continue;
            if (memcmp(node->e[i].key.data, key_data, key_len) != 0) continue;
            /* cocok — hapus */
    ...
    for (int i = 0; i < node->n; i++) {
        if (!rect_intersects(&node->e[i], &bounds, ndims)) continue;
        if (rtree_delete_rec(node->e[i].child, rect, key_data, key_len, ndims)) {
            /* perbarui MBR anak */
            node_recompute_mbr(node->e[i].child,
                node->e[i].min, node->e[i].max, ndims);
            return true;
        }
    }
    return false;
...
/* query rekursif */
static fastkv_err_t rtree_within_rec(rnode_t *node, const fastkv_rect_t *b,
    fastkv_rtree_cb cb, void *udata, int ndims)
{
    for (int i = 0; i < node->n; i++) {
        if (!rect_intersects(&node->e[i], b, ndims)) continue;
        if (node->is_leaf) {
            if (!rect_within(&node->e[i], b, ndims)) continue;
            fastkv_rect_t r;
...
static fastkv_err_t rtree_intersects_rec(rnode_t *node, const fastkv_rect_t *b,
    fastkv_rtree_cb cb, void *udata, int ndims)
{
    for (int i = 0; i < node->n; i++) {
        if (!rect_intersects(&node->e[i], b, ndims)) continue;
...
static heap_item_t heap_pop(minheap_t *h)
{
    ...
    while (true) {
        ...
        if (l < h->n && h->data[l].dist < h->data[smallest].dist) smallest = l;
        if (r < h->n && h->data[r].dist < h->data[smallest].dist) smallest = r;
        if (smallest == i) break;
...
{
    if (!out || ndims == 0 || ndims > RTREE_MAX_DIMS) return FASTKV_ERR_INVAL;

    fastkv_rtree_t *t = fkv_malloc(sizeof(*t));
    if (!t) return FASTKV_ERR_NOMEM;
...
{
    if (!t || !key.data) return FASTKV_ERR_INVAL;

    uint8_t *kdata = fkv_malloc(key.len);
    if (!kdata) return FASTKV_ERR_NOMEM;
...
{
    if (!t || !cb) return FASTKV_ERR_INVAL;
...
fastkv_err_t fastkv_rtree_nearby(fastkv_rtree_t *t, fastkv_coord_t *point, uint64_t limit,
    fastkv_rtree_cb cb, void *udata)
{
    if (!t || !point || !cb) return FASTKV_ERR_INVAL;
